...
proc parseFn(start: int, ctx: Context): (Node, int, bool)
...
proc parsePointerType(start: int, ctx: Context): (Node, int, bool)
proc parseArg(start: int, ctx: Context): (Node, int, bool)
...
proc parseType(start: int, ctx: Context): (Node, int, bool) =
  log(Type)
  result = parsePointerType(start, ctx)
  if not result[2]:
...
proc parseArgs(start: int, ctx: Context): (Node, int, bool) =
  ...
  while true:
    ...
    (child0, i0, success0) = parseArg(i3, ctx)
    if success0:
...
proc parseFn(start: int, ctx: Context): (Node, int, bool) =
  ...
  (child, i, success) = parseLit("fn ", i, ctx)
  if not success:
  
    finalLog(Fn);return (nil, start, false)
  if not child.isNil: children.add(child)
  (child, i, success) = parseName(i, ctx)
  if not success:
  ...
  if not success:
    ...
    finalLog(Fn);return (nil, start, false)
  if not child.isNil: children.add(child)
  result = (Fn.init(children), i, true)
  finalLog(Fn)
...
proc parsePointerType(start: int, ctx: Context): (Node, int, bool) =
  ...
  if not success:
    ...
    finalLog(PointerType);return (nil, start, false)
  if not child.isNil: children.add(child)
  (child, i, success) = parseTypename(i, ctx)
  if not success:
    ...
    finalLog(PointerType);return (nil, start, false)
  if not child.isNil: children.add(child)
  result = (PointerType.init(children), i, true)
  finalLog(PointerType)
...
proc parseArg(start: int, ctx: Context): (Node, int, bool) =
  ...
  if not success:
    ...
    finalLog(Arg);return (nil, start, false)
  if not child.isNil: children.add(child)
  (child, i, success) = parseLit(":", i, ctx)
  if not success:
  ...
  if not success:
    ...
    finalLog(Arg);return (nil, start, false)
  if not child.isNil: children.add(child)
  (child, i, success) = parseType(i, ctx)
  if not success:
  
    finalLog(Arg);return (nil, start, false)
  if not child.isNil: children.add(child)
  result = (Arg.init(children), i, true)
  finalLog(Arg)
